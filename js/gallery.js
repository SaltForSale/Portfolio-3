...
function swapPhoto() {
	...
	name.innerHTML = mImages[mCurrentIndex].name;
	//change location
	let description = document.getElementsByClassName('description')[0];
	description.innerHTML = mImages[mCurrentIndex].description;
	//change description
	let link = document.getElementsByClassName('link')[0];
	link.innerHTML = mImages[mCurrentIndex].link;
	//change date
	mLastFrameTime = 0;
	mCurrentIndex += 1;
...
function iterateJSON(mJson) {
	for (x = 0; x < mJson.images.length; x++) {
		...
		mImages[x].link = mJson.images[x].link;
		mImages[x].img = mJson.images[x].imgPath1;
        mImages[x].img = mJson.images[x].imgPath2;
	}
}
